...
import { Briefcase, GraduationCap, Award, Layers, User, Loader2, Sparkles, Download, Edit3, ImagePlus } from 'lucide-react';
...
// Initialize with the user's provided data
...
const App: React.FC = () => {
  ...
  // Default to 'minimal-slate' for a simple, professional look
  ...
  const handleGenerate = async () => {
    if (!inputText.trim()) {
      setError("Please enter some text to update your resume.");
      return;
    }
    ...
    try {
      ...
      setError("Failed to update. Please check your API Key in Netlify settings.");
      console.error(err);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleDownloadPdf = async () => {
    ...
    if (html2pdf) {
        const opt = {
            margin: 0,
            filename: `Resume-${resumeData?.name.replace(/\s+/g, '-') || 'CV'}.pdf`,
  ...
  // --- RENDER: INPUT MODE (For AI Updates) ---
  if (inputMode) {
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-4">
        <div className="max-w-3xl w-full bg-white rounded-xl shadow-lg p-8">
          <div className="flex items-center gap-3 mb-6">
            <Edit3 className="text-gray-700" size={32} />
            <h1 className="text-3xl font-bold text-gray-900">Update Resume Data</h1>
          </div>
          
          <p className="text-gray-600 mb-6">
            Paste your updated info below to refresh the CV content.
          </p>

          <textarea
            className="w-full h-64 p-4 border border-gray-300 rounded-lg focus:ring-2 focus:ring-slate-500 focus:border-transparent outline-none font-mono text-sm mb-4 resize-y"
          ...
          <div className="flex gap-4">
            <button
              ...
              disabled={isLoading}
              className="flex-1 bg-slate-800 hover:bg-slate-900 text-white font-semibold py-3 px-6 rounded-lg transition-colors flex items-center justify-center gap-2 disabled:opacity-50"
            >
              {isLoading ? (
                <>
    ...
    <div className={`min-h-screen p-4 md:p-12 print:p-0 transition-colors duration-500 ${currentTheme.colors.bg} ${currentTheme.font.body}`}>
      ...
      <div className="fixed top-4 right-4 flex flex-col gap-2 print:hidden z-50">
        <button
          ...
          className="bg-white shadow-md p-3 rounded-full hover:bg-gray-100 text-gray-700 tooltip border border-gray-200"
          title="Edit Data with AI"
        >
          <Edit3 size={20} />
        </button>
        <button
        ...
        <button
          ...
          className={`${isGeneratingPdf ? 'bg-gray-400' : 'bg-slate-800 hover:bg-slate-900'} shadow-md p-3 rounded-full text-white tooltip transition-colors border border-transparent`}
          title="Download PDF"
        >
          {isGeneratingPdf ? <Loader2 size={20} className="animate-spin" /> : <Download size={20} />}
        </button>
        <label className="bg-white shadow-md p-3 rounded-full hover:bg-gray-100 text-gray-700 cursor-pointer tooltip border border-gray-200" title="Upload Photo">
           <ImagePlus size={20} />
      ...
      <footer className="max-w-4xl mx-auto mt-8 text-center text-gray-400 text-sm no-print">
        <p>Vo Nguyen Huy - {new Date().getFullYear()}</p>
      </footer>
    </div>
  );
};
